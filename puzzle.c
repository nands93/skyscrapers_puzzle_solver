...
/*   Updated: 2023/07/10 16:18:00 by femarque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int fill_grid(int row, int col, int* restrictions, int grid[SIZE][SIZE])
{
    ...
    if (row == SIZE)
    {
        if (check_restrictions(restrictions, grid))
            return (1);
        return (0);
    }
    if (col == SIZE)
        return (fill_grid(row + 1, 0, restrictions, grid));
    while (value <= SIZE)
    {
        if (is_valid(row, col, value, grid))
        {
            grid[row][col] = value;
            if (fill_grid(row, col + 1, restrictions, grid))
                return (1);
            grid[row][col] = 0;
        }
        value++;
